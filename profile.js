...
document.querySelector("[data-account-wishlist]").innerHTML = wishlist.data
  ...
  ? wishlist.data
      .map(
        (item) =>
          `<a class="account-item" href="product.html?product=${item.products.slug}"><strong>${item.products.name}</strong><b>Rs. ${Number(item.products.price).toLocaleString("en-PK")}</b></a>`,
      )
      .join("")
