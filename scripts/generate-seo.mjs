  ...
  for (const product of DEFAULT_PRODUCTS.filter(
    ...
    if (isService) {
      html = html
        ...
        .replace(/<span data-city-label>[\s\S]*?<\/span>/i, `<span data-city-label>Event Town / Area (Karachi)</span>`)
        .replace(/placeholder="e\.g\.\s*Nazimabad,\s*Gulshan,\s*DHA\s*Karachi"/i, `placeholder="e.g. Gulshan, DHA, North Nazimabad, Karachi"`)
        .replace(/<span data-date-label>[\s\S]*?<\/span>/i, `<span data-date-label>Event Date</span>`)
        .replace(/<span data-payment-label>[\s\S]*?<\/span>/i, `<span data-payment-label>Payment Preference</span>`)
        .replace(/<select class="field" name="payment" required>[\s\S]*?<\/select>/i, `<select class="field" name="payment" required><option value="">Select payment</option><option>50% Advance Bank Transfer</option><option>50% Advance JazzCash</option><option>50% Advance Easypaisa</option><option>Cash on Setup</option></select>`)
        .replace(/<span data-address-label>[\s\S]*?<\/span>/i, `<span data-address-label>Venue Name &amp; Address</span>`)
        .replace(/placeholder="e\.g\.\s*House\s*#,\s*Street\s*#,\s*Sector\s*\/\s*Block,\s*Area,\s*Karachi"/i, `placeholder="e.g. Banquet Name / Lawn / Home Address, Karachi"`)
        .replace(/<span data-note-label>[\s\S]*?<\/span>/i, `<span data-note-label>Special Customization Requests</span>`)
        .replace(/placeholder="e\.g\.\s*Custom\s*greeting\s*card\s*message,\s*ribbon\s*color\s*preference\.\.\."/i, `placeholder="e.g. Stage dimensions, dress color theme, fairy lights..."`)
      ...
      html = html
        ...
        .replace(/<span data-city-label>[\s\S]*?<\/span>/i, `<span data-city-label>Delivery Town / Area (Karachi)</span>`)
        .replace(/<span data-date-label>[\s\S]*?<\/span>/i, `<span data-date-label>Delivery Date</span>`)
        .replace(/<span data-payment-label>[\s\S]*?<\/span>/i, `<span data-payment-label>Payment Method</span>`)
        .replace(/<select class="field" name="payment" required>[\s\S]*?<\/select>/i, `<select class="field" name="payment" required><option value="">Select payment</option><option>Bank Transfer</option><option>JazzCash</option><option>Easypaisa</option><option>Cash on Delivery (50% Advance)</option></select>`)
