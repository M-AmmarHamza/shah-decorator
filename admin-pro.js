  ...
  function render() {
    ...
    panel(
      ...
      `<form class="pro-settings" data-settings-form>
        <div class="content-form-row"><label>Business name<input class="field" name="businessName" value="${esc(settings.businessName || "PakMarket")}"></label><label>Store tagline<input class="field" name="tagline" value="${esc(settings.tagline || "Seamless WhatsApp Ordering")}"></label></div>
        ...
        <fieldset class="theme-picker"><legend>Store colour theme</legend><p>Selecting a theme instantly updates the storefront preview.</p><div>${STORE_THEMES.map((theme) => `<label style="--theme-colour:${theme.primary};--theme-soft:${theme.soft}"><input type="radio" name="theme" value="${theme.id}" ${(settings.theme || "emerald") === theme.id ? "checked" : ""}><span><i></i><b>${theme.name}</b><small>${theme.primary}</small></span></label>`).join("")}</div></fieldset>
        <div class="content-form-row"><label>Custom primary colour<input class="field" type="color" name="primaryColor" value="${esc(settings.primaryColor || "#007a55")}"><small>Selecting a preset will automatically update this color.</small></label><label>Default delivery charge (Rs.)<input class="field" type="number" min="0" name="deliveryFee" value="${esc(settings.deliveryFee || "0")}"></label></div>
        ...
        <details><summary>Payment and advanced settings</summary><div class="content-form-row"><label>Easypaisa/JazzCash<input class="field" name="wallet" placeholder="Account number" value="${esc(settings.wallet || "")}"></label><label>Account title<input class="field" name="accountTitle" value="${esc(settings.accountTitle || "")}"></label></div></details>
        <button class="admin-btn primary">Save settings</button>
      </form>`,
    );
    panel(
      "media",
