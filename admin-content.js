(() => {
  ...
  function fieldsFor(type) {
    if (type === "events")
      return `<input type="hidden" name="id"><div class="content-form-row">${input("Event name", "title", "text", "required maxlength=80")}${input("Discount code", "code", "text", "maxlength=30")}</div>${input("Main announcement", "message", "text", "required maxlength=140")}${input("Secondary message", "secondary", "text", "maxlength=100 placeholder='e.g. Free delivery above Rs. 3,000'")}<div class="content-form-row"><label>Discount type<select name="discountType" required><option value="percent">Percentage (%)</option><option value="fixed">Fixed amount (Rs.)</option></select></label>${input("Discount value", "discountValue", "number", "required min=0 step=0.01")}</div><div class="content-form-row">${input("Minimum order (Rs.)", "minimumOrder", "number", "min=0 value=0")}<label>Apply offer to<select name="productScope" required><option value="all">All products</option><option value="category">One category</option><option value="products">Selected products</option></select></label></div><div class="content-form-row"><label>Target category<select name="targetCategory">${window.PakMarketCategories.options()}</select></label><label>Selected products<select name="productIds" multiple size="5">${(JSON.parse(localStorage.getItem("pakmarket_inventory_v1")||"[]")).map(product=>`<option value="${esc(product.id)}">${esc(product.name)} · ${esc(product.sku)}</option>`).join("")}</select><small>Use Ctrl/Cmd to select multiple products.</small></label></div><div class="content-form-row"><label>Delivery treatment<select name="deliveryMode" required><option value="default">Use website default</option><option value="included">Included in shown price</option><option value="separate">Charge separately</option><option value="free">Free delivery</option></select></label>${input("Delivery charge (Rs.)", "deliveryFee", "number", "min=0 value=0")}</div><div class="content-form-row">${input("Start date & time", "start", "datetime-local", "required")}${input("End date & time", "end", "datetime-local", "required")}</div>${check("Enabled", "enabled", "Campaign can run during its scheduled window.")}`;
    if (type === "coming")
  ...
  function openEditor(type, id) {
    ...
    if (type === "events" && form.elements.productIds)
      [...form.elements.productIds.options].forEach(
        (option) => (option.selected = (item.productIds || []).includes(option.value)),
      );
    if (type === "blogs")
  ...
  document
    ...
    .addEventListener("submit", (event) => {
      ...
      if (type === "events")
        data.productIds = new FormData(form).getAll("productIds");
      if (
